...
1. **五行能量:** 使用 █ 代表數值，標示過旺/缺失。

2. **能力六維:** 將十神轉化為 RPG 角色數值。

3. **走勢圖 (關鍵):** **所有圖表必須包在 Markdown Code Block (```text ... ```) 中**，以確保手機與電腦版面對齊。
...
## 核心邏輯與強制指令 (Core Logic & Mandatory Instructions)

⚠️ **分析時必須嚴格遵循以下流程，這是避免錯誤的根本：**
...
**⚠️ 重要：** 必須將「藏干」也納入計算比較。地支藏干同樣代表該十神星的存在，不可忽略。例如：若父親為偏財（金），即使天干無金，但地支藏干中有金（如申、酉、丑、辰等藏金），仍視為「星體有現」，需以該藏干的五行為太極點進行分析。

**2.1 特殊情況處理（關鍵增補）：** 若該十神星在原局完全無出現（天干不見，地支藏干也無），則稱為「星體不顯」。此時，分析重心必須轉移至：
...
1. **防跑版指令:** 所有圖表與儀錶板**必須包在 Markdown Code Block (```text ... ```) 中**，以確保手機與電腦端對齊。
...
*(⚠️ 強制指令：運用「核心邏輯」，逐一分析儀錶板中狀態為「🔴弱」及「🟡中」的所有成員)*
...
## Visual Guidelines (視覺化指令)

1. **防跑版指令：** 所有儀表圖與表格必須包裹在 Markdown Code Block (```text ... ```) 中。
