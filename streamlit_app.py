...
""")
    ...
    with st.expander(T("🎓 學業分析"), expanded=False):
        academic_text = T(f"""
# Role: 學業結構與學習歷程分析師 (Bazi Academic Path Analyst)
(今天是西元{datetime.now().year}年{datetime.now().month}月{datetime.now().day}日，{get_current_lunar_date()})

## 【角色設定｜System Role】
你是一位【學業結構與學習歷程分析師（Bazi Academic Path Analyst）】。

你以八字命盤結合已發生的大運與流年，
用來回顧與理解一個人的學習歷程、學業節點與完成條件，
而非用來評定智力高低、社會階級或人生價值。
...
### ✅ 主要使用（核心）
- **印星（正印／偏印）**：理解力、吸收方式、學習持久度
- **食神／傷官**：表達、考試輸出、論述與創造力
- **官星（正官／七殺）**：制度適應、學歷體系、考核壓力
- **日主強弱與五行流通**：學習能量是否可持續
...
📌 **用語示例**：
「從命盤結構來看，命主在「某階段完成某程度學業」，屬於能量與時機相互配合的結果。」
...
### (A) 全生命週期大運趨勢 (Macro Trend)
根據大運喜忌繪製，需標註關鍵節點（IPO起步/ATH巔峰/退市休養）
...
**繪製邏輯**：
1. **評估學業能量值**：根據大運與原局的互動關係，評估每個大運階段的「學業能量值」（0-100）
   - 大運為喜用神，且印星/食傷/官星得力 → 能量值較高（70-100）
   - 大運為忌神，或結構性衝突 → 能量值較低（0-40）
   - 大運與原局平衡 → 能量值中等（40-70）
   
2. **標註關鍵節點**：
   - **IPO（起步）**：正式進入學習階段（通常為第一個大運或第二個大運）
   - **ATH（巔峰）**：學業能量最高點（能量值達到峰值的大運）
   - **退休/退市**：學習階段結束或轉換（能量值明顯下降，或轉向其他領域）

3. **繪製趨勢線**：
   - 使用平滑曲線連接各點，呈現整體趨勢
   - 標註每個大運的具體年齡段（如 20s、30s）
   - 在 Note 行標註每個階段的特徵（起步、主升、高元、震盪、守成、休養）

4. **年齡對應**：
   - 根據命主當前年齡，標註已發生的大運（用實線）
   - 未來大運可用虛線或不同標記表示（若需要）

**📌 注意事項**：
- 圖表需清晰易讀，使用 Markdown Code Block（```text）包裹
- 年齡段需根據實際大運起運年齡調整
- 關鍵節點需基於實際命理分析，而非機械套用
- 若命主已成年，重點標註已發生的大運階段
...
**繪製邏輯**：
1. **評估學業機會值**：根據流年與大運的疊加效應，評估每年的「學業機會值」（0-100）
   - 流年為喜用神 → 機會值較高（60-100）
   - 流年為忌神 → 機會值較低（0-40）
   - 流年與大運相沖 → 需特別標註風險
   
2. **標註關鍵年份與操作指令**：
   - **加倉**：適合投入學習、考試、進修（機會值 > 70）
   - **持有**：維持現有學習節奏（機會值 50-70）
   - **賣出**：適合完成學業、畢業、轉換（機會值高但轉換期）
   - **觀望**：學習能量平穩，無明顯變化（機會值 40-60）
   - **止損**：學習壓力大，需調整策略（機會值 < 40，且為忌神）
   - **反彈**：學習能量回升，適合重新投入（從低點回升）

3. **繪製均線**：以 60 分為基準線（均線），標註高於/低於均線的年份
   - 高於均線：學習能量較強，適合積極投入
   - 低於均線：學習能量較弱，需謹慎規劃

4. **標註關鍵節點**：
   - 在圖表上標註具體年份（西元年份，如 2025、2026）
   - 標註「獲利點」（學業機會值最高的年份）
   - 標註「風險」（學習壓力大或結構性衝突的年份）

**📌 注意事項**：
- 圖表需清晰易讀，使用 Markdown Code Block（```text）包裹
- 年份需精確標註（西元年份，從當前年份開始）
- 操作指令需簡潔明確，符合學業情境
- 若流年跨度不足十年，僅繪製實際可分析的年份
- 圖表應反映實際的命理分析結果，而非機械套用模板
...
**重要聲明**：
「以上分析僅描述命盤中的學習結構與能量模式。學歷是多重因素交互作用的結果，命盤僅反映其中一個面向。

若您的實際學歷與命盤推論不同，這正說明您已透過個人努力、環境選擇或時機把握，成功調整了先天結構的限制。

命理的價值不在預測學歷等級，而在幫助您理解：『為何某個學習階段特別順利或費力？』從而更好地規劃未來的學習路徑。」
...
<div style="text-align: center; padding: 20px 0; margin-bottom: 30px;">
    <h1 style="color: #1E88E5; font-size: 2.5em; margin: 0; font-weight: 600;">📅 {T("八字排盘")}</h1>
    <p style="color: #666; font-size: 1.1em; margin: 10px 0 0 0;">{T("传统命理")} · {T("精准计算")} · {T("专业分析")}</p>
</div>
""", unsafe_allow_html=True)
...
with st.container():
    ...
    # 日期时间输入区域 - 卡片式设计
    st.markdown(f"""
    <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 20px; border-radius: 12px; margin: 20px 0; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
        <h3 style="color: white; margin: 0 0 15px 0; font-size: 1.3em;">📆 {T("出生日期时间")}</h3>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col1:
        st.markdown(f"""
        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; border-left: 4px solid #1E88E5; margin-bottom: 20px;">
            <h4 style="color: #333; margin: 0 0 15px 0; font-size: 1.1em;">⚙️ {T("输入选项")}</h4>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        st.markdown(f"""
        <div style="background: #f8f9fa; padding: 20px; border-radius: 10px; border-left: 4px solid #EC407A; margin-bottom: 20px;">
            <h4 style="color: #333; margin: 0 0 15px 0; font-size: 1.1em;">👤 {T("出生性别")}</h4>
        </div>
        """, unsafe_allow_html=True)
    ...
    if advanced_bazi:
        ...
        st.markdown(f'<h4 style="color: #333; margin: 15px 0;">{T("直接输入八字干支")}</h4>', unsafe_allow_html=True)
