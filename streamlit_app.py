...
- **認知與反應模式**：當你遇到壓力或機會時，你的第一反應是什麼？你的思考邏輯是什麼？用具體場景描述，不要只說「理性」或「感性」。

- **人際與情緒機制**：你在人際關係中如何設定邊界？你的情緒如何流動？這些如何影響你的現實生活？用具體例子說明。

- **性格張力與盲點**：你性格中的矛盾點是什麼？例如：
  - 如果遇到「財印相戰」：不要只說術語，要說「你內心有兩種力量在拉扯：一邊想要追求現實利益和物質（財），一邊想要學習、名譽和安全感（印），就像一個人在理想和現實之間掙扎」
  - 如果遇到「身強無洩」：要說「你的能量很強，但找不到合適的出口，就像一個精力充沛的人被困在房間裡，容易內耗、焦慮，需要找到釋放能量的方式（比如運動、創作、工作）」
...
- **星**：十神所代表六親
- **宮**：四柱六親宮位（年柱=祖輩/父、月柱=父母/兄弟、日支=配偶、時柱=子女）
- **運**：大運（兼看流年）

👉 **三者缺一，不可妄斷**。必須同時考慮十神、宮位、大運流年，才能準確判斷六親關係。

**② 以日主喜忌為綱**

- **六親星為日主喜用** → 吉（該六親對命主有利，帶來助力、提攜）
- **六親星為日主忌神** → 凶（該六親對命主不利，帶來拖累、限制）
- **喜神受制** → 反不吉（原本有利的六親，因受剋制而無法發揮作用）
- **忌神受制** → 反為吉（原本不利的六親，因受剋制而減少負面影響）

**③ 以六親為太極反斷**

- **六親星旺** → 取剋洩耗為其用（該六親能量強，需要制衡）
- **六親星弱** → 取生扶為其用（該六親能量弱，需要幫助）
- **六親星弱極** → 從象論，從格取用（該六親能量極弱，需順應其勢）
...
- **越旺 + 越近日主** → 影響越大（能量強且距離近，對命主影響深遠）
- **越弱 + 越遠日主** → 影響越小（能量弱且距離遠，對命主影響較小）
- **喜用六親** → 助力、提攜（對命主有利的六親，帶來正面影響）
- **忌神六親** → 拖累、限制（對命主不利的六親，帶來負面影響）
...
- **歲運改 → 用神變 → 六親喜忌亦變**（大運流年改變，日主喜忌可能改變，六親的吉凶也會改變）
- **先吉後凶 / 先凶後吉，皆因歲運**（同一六親在不同時期作用不同，因為歲運在變化）
- **同一六親，不同時期作用不同**（需要結合當前大運流年來判斷六親的實際作用）

### 四、位正判斷提示（性情品行）

**位正論賢，不論富貴壽夭**：

- **父位父守** → 父端正（年柱為父星，且為喜用，父親品行端正）
- **母位母守** → 母賢良（月柱為母星，且為喜用，母親賢良）
- **妻位妻守** → 妻守分（日支為妻星，且為喜用，妻子守分）
- **子位子守** → 子端正（時柱為子星，且為喜用，子女端正）

📌 **位正 ≠ 富貴**（位正只論品行，不論財富地位）
📌 **位正 ≠ 喜用**（位正與喜用是兩個不同的判斷維度）
📌 **專論 節操、性情、品行**（位正主要判斷六親的道德品質，而非其對命主的實際作用）
...
- **年柱為喜用** → 得祖蔭
  - **天干喜** → 明蔭（錢物、可見的幫助）
  - **地支喜** → 暗蔭（風水、隱性的助力）
  - **干支皆喜** → 明暗俱全（既有物質幫助，也有隱性助力）
- **年柱為忌** → 無蔭反累（得不到祖上幫助，反而受拖累）

**② 父母庇蔭（月柱）**

- **月柱喜用** → 得父母助（父母對命主有幫助）
- **月柱忌神** → 父母拖累（父母對命主不利）
  - **忌神** → 因病、災、家庭事誤己（可能因為父母生病、災難、家庭事務而影響命主）

**③ 兄弟助力（日柱）**

- **日柱喜用** → 得兄弟之助（兄弟對命主有幫助）
- **日柱忌神** → 兄弟牽累（兄弟對命主不利，造成牽累）

**④ 子女反饋（時柱）**

- **時柱喜用** → 子女孝順（子女對命主孝順，有幫助）
- **時柱忌神** → 子女不力或拖累（子女對命主不利，或不孝順，或造成拖累）

### 六、宮位速斷總訣

**某宮為喜，得其恩；某宮為忌，受其累；力量大小，看其旺衰。**

- 年柱為喜用 → 得祖上恩惠；為忌 → 受祖上拖累
- 月柱為喜用 → 得父母恩惠；為忌 → 受父母拖累
- 日支為喜用 → 得配偶恩惠；為忌 → 受配偶拖累
- 時柱為喜用 → 得子女恩惠；為忌 → 受子女拖累

**力量大小**：需看該宮位的旺衰程度，旺則影響大，衰則影響小。

### 七、終極速斷心法

**星定其人，宮定其位，喜忌定其情，歲運定其應。**

- **星定其人**：十神決定六親的類型與特質
- **宮定其位**：宮位決定六親的關係位置
- **喜忌定其情**：喜忌決定六親對命主的情感與作用（有利或不利）
- **歲運定其應**：歲運決定六親在特定時期的實際表現與影響
...
### 1. 家族系統能量總覽
- 命主性別 + 日主五行 + 喜用神與忌神
- 六親十神定位表（標註：顯著度等級、宮位、喜忌狀態、元氣狀態）
- **判斷依據**：
  - **星·宮·運同參**：標註每個六親的十神（星）、宮位（宮）、當前大運影響（運）
  - **喜忌判斷**：標註該六親星為日主喜用或忌神，及其對命主的實際作用
  - **庇蔭分析**：標註年柱、月柱、日支、時柱的喜忌，判斷祖上、父母、配偶、子女的庇蔭情況
- **元氣評級標準**：
...
- **邏輯推演**：
  1. **星·宮·運同參**：十神定位（星）→ 宮位分析（宮）→ 大運流年影響（運）
  2. **喜忌判斷**：該六親星為日主喜用或忌神 → 對命主的實際作用（助力/拖累）
  3. **太極反斷**：以該六親為太極，分析其旺衰與用神需求
  4. **位正判斷**：該六親是否「位正」，判斷其性情品行
  5. **庇蔭分析**：該六親所在宮位是否為喜用，判斷其庇蔭程度
- **歲運變化**：結合當前大運流年，說明該六親在不同時期的實際作用（先吉後凶/先凶後吉）
- **命主感受**：基於能量模式，推測「命主在此互動中可能產生的內在感受」（使用「可能」「傾向」，避免推測他人動機）

### 3. 系統調整建議（簡要即可）
- **關係課題**：命主在家族中的核心關係議題（基於星·宮·運同參的綜合判斷）
- **庇蔭利用**：如何利用年柱、月柱、日支、時柱的喜用能量，獲得相應的庇蔭
- **歲運應對**：根據當前大運流年，調整與不同六親的互動策略（喜用六親加強互動，忌神六親保持距離）
- **能量調整建議**：基於五行的微小行動建議
