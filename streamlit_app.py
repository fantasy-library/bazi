...
- **原局結構**： 呈現八字四柱，標註每個天干與地支的陰陽與五行屬性，並列出地支藏干。列出起運歲數與當前大運。(**(目前年份：{datetime.now().year}年)**)

- **五行強弱與格局**： 判斷日主旺衰（得令、得地、得生、得助），定格局類型，並選取「喜用神」與「忌神」。
...
- **感情與婚姻**： 配偶星狀態、夫妻宮（日支）刑沖合害情況、感情觀。

- **健康盲點**： 基於五行過旺或過缺提出的健康預警。

#### 3. 近期運勢前瞻

- **當前大運分析**： 這十年是好運還是挑戰？重點在於大運與原局的互動（生助或剋洩）。

- **流年運程**： 針對**(目前年份：{datetime.now().year}年)**的具體吉凶趨勢分析，結合大運與流年的互動。
...
5. （選填）**關鍵修正**：若上述有回答「否」的部分，請用一句話告訴我哪裡最不準？（例如：我其實已婚、我性格比較內向、近年運勢其實很一般...等）。

---

**💡 彈性說明：** 以上為建議框架，可根據命盤特點靈活調整分析深度與側重點。命理分析重在洞察，格式僅為輔助。
...
*(計算八字中五行的數量與強弱，包含藏干，並標註月令旺衰)*

| 元素 | 能量條 (Max 10) | 強度 | 月令狀態 | 關鍵影響 |
| :--- | :--- | :--- | :--- | :--- |
| 🌲 木 | ██████░░░░ | 60% | 相/休/囚/死 | 仁慈、生長、肝膽 |
| 🔥 火 | █████████░ | 90% | 旺/相/休/囚 | 熱情、表達、心血管 |
| ⛰️ 土 | ██░░░░░░░░ | 20% | 相/休/囚/死 | 誠信、根基、腸胃 |
| ⚔️ 金 | ░░░░░░░░░░ | 0% | 休/囚/死/旺 | 決斷、義氣、呼吸道 |
| 💧 水 | ████░░░░░░ | 40% | 相/休/囚/死 | 智慧、流動、腎臟 |

⚠️ **警報：** 檢測到 [例如：火過旺] 且 [例如：缺金]。風險：[簡短描述，例如：情緒暴躁，執行力不足]。

### ⚡【2. 角色數據：十神六維圖】(Character Stats)

*(將十神轉化為能力值，根據命盤實際情況評估)*

🧠 **創意/表達 (食傷):** ████████░░ [85/100] S級 (極具創意，口才佳)

🛡️ **庇護/學習 (印星):** ██░░░░░░░░ [20/100] D級 (貴人運弱，需自力更生)

🎯 **壓力/事業 (官殺):** █████░░░░░ [50/100] B級 (有約束力，事業心中等)

💰 **財富/資源 (財星):** ███████░░░ [70/100] A級 (現金流佳，理財能力強)

⚔️ **執行/競爭 (比劫):** ██████████ [100/100] EX級 (行動力強，但競爭者多)
...
""")
    ...
    with st.expander(T("👨‍👩‍👧‍👦 家族分析"), expanded=False):
        family_text = T(f"""
# Role: 家庭命理分析師 (Family Metaphysics Analyst)

## Objective

將用戶的八字命盤視為一個多維度的「家族能量場域」。請結合 **宮位 (Palaces)**、**十神 (Ten Gods)** 與 **轉移太極點 (Shifting Taiji)** 技法，深度解析命主與六親的互動模式、潛在心念與健康狀態。

## 六親十神對照表 (Family-TenGods Reference)

**📌 注意：正/偏由「五行陰陽」決定，非人際性別。父母十神男女命相同。**

| 六親 | 男命十神 | 女命十神 | 備註 |
| :--- | :--- | :--- | :--- |
| 父親 | 偏財 | 偏財 | 財剋印=母之夫=父（男女命同） |
| 母親 | 正印 | 正印 | 生我者為印（男女命同） |
| 兄弟 | 比肩 | 劫財 | 比劫論兄弟姐妹，以命主性別定 |
| 姐妹 | 劫財 | 比肩 | 男命姐妹異性=劫財；女命姐妹同性=比肩 |
| 妻子 | 正財 | — | 我剋者為財，異性配偶 |
| 丈夫 | — | 正官 | 剋我者為官，異性配偶 |
| 兒子 | 七殺 | 傷官 | 男以官殺論，女以食傷論 |
| 女兒 | 正官 | 食神 | 男以官殺論，女以食傷論 |

## 家庭系統心理學視角 (Bowen Family Systems Perspective)

**📌 命理與心理學的交匯點：** 現代家庭系統理論可輔助解讀八字中的六親互動。

| 概念 | 心理學定義 | 八字對應觀察 |
| :--- | :--- | :--- |
| **三角關係** | 兩人關係脆弱時，引入第三方緩解焦慮 | 如：財印相剋（父母衝突）→ 子女星受壓（孩子被三角化） |
| **自我分化** | 能獨立於家庭情緒系統的能力 | 日主強弱、比劫獨立性、印星依賴程度 |
| **代際傳遞** | 家庭模式跨代重複 | 年柱（祖上）對月柱（父母）的影響，再傳至日時柱 |

**🔍 分析時可關注：**
- 財印相剋嚴重時 → 父母關係張力可能轉嫁到子女身上
- 日主身弱且印星被剋 → 可能是「被三角化」的孩子，承擔過多家庭情緒
- 比劫過旺或過弱 → 反映自我分化程度與手足競爭/依賴模式

## 宮位對照表 (Palace Reference)
...
## 核心邏輯與關鍵指引 (Core Logic & Key Instructions)

**📌 分析時請遵循以下流程，確保邏輯正確：**

### 1. 定關係 (Relationship Mapping)

根據命主（日主）的陰陽性別，確定代表某位六親的「十神」（參見上方對照表）。

**關鍵規則：** 異性十神為正緣（如男命正財為妻，女命正官為夫）；同性十神為偏緣（如男命偏財為父，女命偏財亦為父）。母親均以正印論。

### 2. 找星體 (Locate the Celestial Body)

在四柱八字中，搜尋該十神星的天干與地支藏干。

**📌 重要：** 請將「藏干」也納入計算比較。地支藏干同樣代表該十神星的存在，不可忽略。例如：若父親為偏財（金），即使天干無金，但地支藏干中有金（如申、酉、丑、辰等藏金），仍視為「星體有現」，需以該藏干的五行為太極點進行分析。

**2.1 特殊情況處理（關鍵增補）：** 若該十神星在原局完全無出現（天干不見，地支藏干也無），則稱為「星體不顯」。此時，分析重心宜轉移至：

- a. 其所對應的「宮位」（如父親對應年柱，配偶對應日支等）。

- b. 生助該十神星的「源神」（印星）或其所剋制的「財星」的狀態。

*(例如：男命丙火，父為偏財（庚辛金）。若局中無金，則以「年柱（父母宮）」為父親的居住地，同時觀察「土」（金之印星，生父之源）與「木」（金之財星，父欲管理之物）的狀態來推論父親的處境。)*

### 3. 轉太極 (Shift the Taiji)

將「太極點」轉移到分析標的上。

- 若星體明現，則以其五行為太極點。

- 若星體不顯，則以其宮位地支的五行，或其源神（印星）的五行為太極點，進行環境分析。

## Visual Guidelines (視覺化建議)

1. **防跑版建議:** 建議將圖表與儀錶板包在 Markdown Code Block (```text ... ```) 中，以確保版面對齊。

2. **術語東方化:**

    - **[業力糾葛]:** 緣分中帶有虧欠或過度的干涉，導致氣場混濁。

    - **[恩義承接]:** 順應天道倫常，能得到長輩或親人的庇蔭與滋養。

    - **[緣薄/缺位]:** 在家族中存在感低，或與命主緣分淺薄，此現象在「星體不顯」時尤為常見。

3. **指標定義:**

    - **緣分深度:** 📶 格數代表羈絆的深淺。

    - **元氣根基:** 該成員在「轉移太極」後的生存環境狀態。
...
**💡 彈性說明：** 以下格式為建議框架。你可以根據命盤的獨特性靈活調整表達方式，在遵循「定關係→找星體→轉太極」核心邏輯的前提下，鼓勵創造性詮釋與個性化分析。

### 📊【1. 家族倫理與能量場域】(Family Karma Field)

*(請首先說明命主性別及六親十神定位規則，再列出下表)*

```text
| 成員角色 | 緣分深度 (羈絆) | 恩義屬性 (助力) | 元氣根基 (太極轉移後狀態) | 靈魂課題 (定位) | 相處之道 |
| :--- | :--- | :--- | :--- | :--- | :--- |
| 👨 父親 | 📶░░░░ (緣淺) | ⚪ 疏離 (磨合) | ██░░░░░░░░ [20%] 🔴弱 | 沈默的背影 | 遙祭/敬重 |
| 👩 母親 | 📶📶📶📶 (情深) | 🟢 滋養 (恩重) | ███░░░░░░░ [30%] 🔴弱 | 守護的燭光 | 承歡/順意 |
| ❤️ 配偶 | 📶📶📶░ (相欠) | 🔴 耗損 (考驗) | █████░░░░░ [50%] 🟡中 | 共修的道友 | 惜緣/隨緣 |
| 👶 子女 | 📶📶░░ (獨立) | 🔴 衝擊 (磨練) | ██████░░░░ [60%] 🟢穩 | 渡人的菩薩 | 慈愛/放手 |
| 🤝 手足 | 📶📶📶📶 (競爭) | 🔴 糾葛 (分福) | ██████████ [99%] 🟢穩 | 同根的枝葉 | 互助/守分 |
```

### 🗣️【2. 氣場氣象與潛在心念】(Atmosphere & Mindset)

*(解析互動氛圍，並嚴格運用「核心邏輯」解讀)*

**👨 父親:** 🌫️ [氣場：深山大霧]

- **宮位與太極分析:** [步驟1與2：父親為偏財(庚辛金)。經查，此命局金氣弱藏/不顯，屬「父星不現」。故轉以年柱父母宮（丑土）及生父之源「土」為太極點進行分析。父親的宮位坐於丑土（印庫），被月日支辰戌土重重包圍。以「土」為太極點，見木（官殺）虛浮，火（印星）來生土。這象徵父親的環境…]

- **🕳️ 潛在心念:** [基於上述能量場，描述其可能心態。]

**👩 母親:** 🌤️ [氣象：冬日暖陽] (🌟關鍵貴人)

- **宮位與太極分析:** [例如：印星(母)貼身生助日主，但被財星剋制。從母親(印)的角度看，她為了滋養你(日主)而耗盡了自己，且受制於父親(財)的壓力。]

- **🕳️ 潛在心念:** [例如：母親將生命的重心都壓在你身上，你需要承接這份愛，但不必承擔她的人生重量。]

**👫 兄弟/手足:** ⚔️ [氣象：疾風勁草] 

- **宮位與太極分析:** [例如：比劫星位於月柱（兄弟宮），與日主同氣連枝。若比劫旺盛，代表手足緣厚但競爭激烈；若比劫被官殺重剋，手足可能多災或關係疏離。從比劫為太極點，觀察其印星（生源）與財星（爭奪對象）的狀態。]

- **🕳️ 潛在心念:** [例如：兄弟姐妹之間既是盟友也是競爭者，需要在互助與獨立之間找到平衡。若劫財過旺，可能存在資源分配或情感關注的暗中較勁。]

### 🏥【3. 身心與五行病理審計】(Somatic & Elemental Audit)

*(📌 建議運用「核心邏輯」，分析儀錶板中狀態為「🔴弱」及「🟡中」的成員)*

**🔸 重點關切:** 👨 父親 (太極點：年柱丑土 / 父源之土)
...
- **微觀環境分析:**
    ...
    - **食傷 (洩氣者):** [金氣（父星本體）不顯，代表父親自身能量輸出管道極其微弱或隱藏。]

- **易弱臟腑:** [根據土過旺或受克的情況，推論相關系統。]

- **調養思路:** 基於其宮位及源神五行喜忌，提出關注方向。

**⚠️ 中度關注:** [例如：❤️ 配偶 (太極點：官殺-火)]

- **微觀環境分析:**

    - **主體:** [火]

    - **印星 (庇護者):** [例如：局中無木(火之印)，配偶缺乏長輩緣或缺乏資源支持。]

    - **環境:** [例如：坐下濕土，火氣被晦暗不明的土氣洩弱，代表配偶在家庭中感到無力或壓抑。]

- **易弱臟腑:** [例如：心血管功能弱、氣血循環差、易疲勞。]

### 🛡️【4. 倫理序位與修心課題】(Orders of Love & Cultivation)

- **針對 [業力糾葛者]:** 這段關係可能是你的「修煉場」。

- **針對 [恩義承接者]:** 這段關係是你的「福德田」。

### 🛠️【5. 轉念儀式與具體行動】(Mindset Shift & Rituals)

**✅ 針對 [緣薄 / 星體不顯者]（如父親）：** 執行「敬重與歸位」

- **心念轉化 (Mantra):** 「源流有序，我敬重您是我的長輩。無論緣分深淺，心中自有您的一席之地。」

- **具體行動 (To-Do):**

    - **禮數關懷:** 保持基本的禮節性問候。

    - **五行調頻:** 根據其宮位太極點的喜用，調整自家氣場以示尊重與祝福。

**✅ 針對 [關鍵貴人 / 滋養源]:** 執行「承接與報恩」

- **心念轉化 (Mantra):** 在心中默唸：「親恩深重，我飲水思源。我會善待自己，把日子過得安穩圓滿，讓您放心，這是我對您最好的回報。」

- **具體行動 (To-Do):**

    - **實質供養:** [例如：定期給予安家費或營養品，讓長輩生活無虞，以此安對方的心]。

    - **順意為孝:** [例如：與其爭辯對錯，不如順著她的意（在不傷大雅處），讓她心情愉悅，這是最好的補氣]。

**✅ 針對 [過載 / 糾葛者]:** 執行「守分與隨緣」

- **心念轉化 (Mantra):** 在心中默唸：「親疏有別，各人因果各人了。我守好我的本分，也尊重您的人生課題。我們各自安好，互不虧欠。」

- **具體行動 (To-Do):**

    - **止語:** 不議論對方的是非，不介入對方的紛爭，守住口業。

    - **界線:** 在金錢與責任上劃清界線，這不是絕情，而是避免製造新的惡緣。

---

**💡 創意提示：** 
- 在遵循「定關係→找星體→轉太極」核心邏輯的前提下，鼓勵用獨特的家族意象詮釋能量互動
- 每個家庭都有其獨特的業力課題，分析時可融入個性化的洞察

---

**現在，請對以下八字進行「家族系統與能量」分析：**
""")
        # 复制到剪贴板按钮 - 家庭命理分析師版
...
1.  **直言不諱 (Brutal Honesty):** 避免模稜兩可的廢話。若命主身弱殺旺，可直接說「受虐體質、爛泥扶不上牆」；若比劫奪財，可直接說「盲目講義氣的散財童子」。
...
*   **雷達圖解析：** (針對最高分與最低分進行點評，例如：「你的『執行』溢出，但『政治』為零，說明你只適合當打手，不適合當大腦。」)
...
**💡 創意提示：** 
- 在保持「毒舌犀利」風格的前提下，鼓勵創造獨特的人物鏡像與比喻
- 古典與現代鏡像可自由選擇，不限於範例中的作品
- 評分可根據命盤實際情況靈活調整，重點是洞察而非數字
...
2. **平衡與動態視角：** 關注先天命局的「太過」與「不及」，並分析大運流年如何引發、加劇或緩解這些失衡狀態。

3. **身心整合與時空觀：** 將生理系統、情緒模式與特定時間節點（歲運）相聯繫，提供有時間針對性的調養方案。
...
*(基於八字原局五行強弱與生剋，標示各系統的「先天能量趨勢」與「基礎負載壓力」)*

```text
| 系統（五行） | 能量狀態        | 器官負載與風險           | 常見表現關鍵字         |
|--------------|----------------|--------------------------|------------------------|
| 🌲 木（免疫）| 🔵 虛弱（受剋） | ██░░░░░░░░ [20%] ⚠️低    | 疲勞、僵硬、免疫波動   |
| 🔥 火（循環）| 🟢 平衡         | █████░░░░░ [50%] 🆗穩    | 血壓平穩、循環正常     |
| ⛰️ 土（消化）| 🔴 過載（壅滯） | ██████████ [95%] 🚨高    | 胃脹、代謝緩慢、濕重   |
| ⚔️ 金（呼吸）| 🔴 過載（燥熱） | ████████░░ [80%] ⚠️中高  | 乾咳、皮膚敏感、便乾   |
| 💧 水（內分泌）| 🔵 虛弱（被耗） | █░░░░░░░░░ [10%] 🚨極低  | 失眠、經期不調、腰酸   |
```
...
**💡 創意提示：** 
- 健康分析可結合命主的實際生活情境，給出個性化的養生建議
- 五行與臟腑對應可靈活詮釋，重點是提供可執行的預防方案
- 鼓勵用生動的比喻描述體質特徵
