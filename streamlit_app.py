...
6. **取象符合現代社會**：比喻與意象需貼近當代生活（職場、人際、理財、健康等），避免封建迷信、性別刻板或人身攻擊式表述
...
**神煞、納音為輔**： 神煞和納音都是輔助參考工具。神煞提供特殊標記和彩蛋信息，納音則從五行音律角度補充命理分析。兩者都應作為輔助判斷，不能取代核心的五行生剋、格局用神等主要分析依據。
...
2. **定日主強弱**：判斷你出生那天的能量（日主）是強還是弱，就像判斷一個人的體質。必須綜合以下四方面：
   - **得令**：日干生於何月？你出生的季節是否適合你的能量（如春天生木、夏天生火為得令）
   - **得地**：地支有無根氣？如長生、祿旺、餘氣、墓庫等，就像你腳下是否有穩固的根基
   - **得生**：天干有無印星生扶？就像有長輩或知識滋養你
   - **得助**：天干有無比劫幫扶？就像有同伴與你並肩
   
   綜合判斷：身強、身弱、還是從格？（從格與專旺格為極少數，需嚴格條件）

3. **審月令旺衰（核心環節）**：月令為命局氣勢最專之主，是取用神的第一依據。
   - **月令為何十神？**（官殺、財星、印星、食傷、比劫）
   - **判斷月令的旺衰程度**（過旺／偏旺／中和／偏弱／過衰）：
     - 是否當令？月令本氣是否為當旺之五行（如寅卯月木旺、巳午月火旺）
     - 同黨多寡與生源：命局中有多少五行生助月令？（同黨=同類五行，生源=能生月令的五行）
     - 是否受損？月令是否被沖、克、合、化削弱？（如相鄰地支沖、天干直接剋月支、合化改變性質）
     - 對日主影響：若月令為忌神且直接壓制日主，即使數量不多，也需強力制化
   - **圍繞月令取用神**：月令過旺或偏旺（氣勢太專）→ 喜克、泄、耗以平衡；月令過衰或偏弱（氣勢衰微）→ 喜生扶、保護，使其發揮應有作用；月令為忌神（如七殺攻身、財星耗身）→ 優先制化月令；月令為喜神（如印星生身、比劫幫身）→ 優先扶助月令。

4. **抓命局核心矛盾**：找出命局中最旺與最弱的五行；日主與月令的關係是相生、相克、相同還是被克？有無關鍵的刑、沖、合、會影響格局？確定主要問題：是月令太旺需制化？日主太弱需生扶？還是兩者交戰需通關？

5. **定原局用神**：立足於命局本身，優先從命局中已存在的干支尋找可用之神。用神作用有三：**扶抑**（扶助日主或抑制過旺之神，尤指月令）、**調候**（寒暖燥濕，如冬生喜火、夏生喜水）、**通關**（化解兩神相戰，如金木相戰喜水通關）。若命局無直接可用之神，則分析哪類五行在大運流年出現時最有益。

6. **日主旺衰與格局結論**：根據上述步驟，確定你的人生「劇本類型」（格局），是正常劇本（正格）還是特殊劇本（變格）。

2.5. **體用做功・精神盈虛（盲派視角）**：這是判斷你生命能量是否有效轉化為現實成就的核心指標。不要只看五行多少，要看能量是否「得其中」——欲望與能力是否匹配，目標與手段是否相應。

   **核心概念（用白話解釋）**：
   - **精（用）**：代表你的欲望、目標、事業與現實成就（財官）。就像你想要達成的目標和想要獲得的東西。
   - **神（體）**：代表你承載目標的能力、心智與能量（日主及印比）。就像你的體力和腦力，是你實現目標的基礎。
   - **關鍵問題**：你的能力（神）是否足以承擔你的目標（精）？就像一個人的體力是否足以完成他想做的事情。

   **四種基本狀態（必須根據具體命盤判斷，不可套用）**：
   - **精神俱足**：目標明確且能力足夠（精旺且神能承擔）→ 就像一個有明確目標且有能力實現的人，容易富貴有成，能量高效轉化為成果。
   - **精旺神枯**：目標很大但能力不足（精旺但神弱）→ 就像一個人想要很多東西，但體力或能力跟不上，容易「得而不能守」，或者因為目標太大而壓力過重。
   - **神旺精衰**：能力很強但目標不明確（神旺但精弱）→ 就像一個體力充沛但不知道要做什麼的人，容易「內耗空轉」，能量找不到出口，容易焦慮或浪費精力。
   - **氣濁神枯**：能量流通不順暢，有阻滯（能量阻滯）→ 就像身體的血液循環不順，容易身心俱疲，命運與健康都受影響。這種情況往往伴隨地支刑穿（如卯辰穿、子未穿等），就像身體的經絡不通。

   **分析要點（舉一反三，不可僵化）**：
   - 觀察日主（神）與財官（精）的關係：是日主能統攝財官，還是財官過重壓制日主？用生活化比喻說明，例如：「就像你的能力（日主）能否駕馭你的目標（財官），還是目標太大讓你感到壓力？」
   - 觀察印星的作用：印星是否能化殺生身，將外界壓力轉為自身資糧？還是印星過重導致「神強而無用」？例如：「就像你能否將外界的挑戰（殺）轉化為學習成長的機會（印生身），還是過度依賴保護（印）而失去行動力？」
   - 觀察地支刑穿的影響：如果有刑穿（如卯辰穿、子未穿等），要說明這如何影響能量的流通，就像身體的經絡不通會影響健康一樣。例如：「你的命盤中有XX穿，就像身體某個部位的經絡不通，容易導致XX方面的問題（如情緒、健康、人際關係等）。」
   - 判斷「得其中」的程度：欲望與能力是否匹配？目標與手段是否相應？能量是否流通並產生有效輸出？用具體的生活場景說明，不要只說術語。

   **最高原則**：命理不在「五行多少」，而在「得其中」。真正的高手，觀命只守一軸：精（目標）與神（能力）如何感應、如何轉化、如何流通。這不是算命，而是通向通透而不內耗的生命修養。

3. **喜用忌神取捨（承接上述定用神）**：根據步驟4、5的結論，明確列出對你有利的能量（喜用神）和對你不利的能量（忌神）。判斷維度包括：**病藥**（命局之病需對症之藥）、**平衡**（過旺需制、過弱需扶）、**調候**（寒暖燥濕）、**通關**（化解相戰）。**行運喜忌**：喜神為幫身、助用神、制化月令忌神之五行；忌神為加重月令之弊、損用神、加劇矛盾之五行。注意：同一五行因輕重、位置不同可能有喜忌轉換，需動態分析。說明哪些能量會干擾你的命局，用生活化比喻解釋。
...
- **性格張力與盲點**：結合月令的雙重影響（內在驅動 vs 外在環境）以及「精神盈虛」的角度來分析。你性格中的矛盾點是什麼？你的目標（精）與能力（神）是否匹配？月令作為內在驅動和外在環境的雙重作用，如何在你身上形成張力？例如：
  - 如果遇到「財印相戰」：不要只說術語，要說「你內心有兩種力量在拉扯：一邊想要追求現實利益和物質（財，代表你的目標），一邊想要學習、名譽和安全感（印，代表你的能力基礎），就像一個人在理想和現實之間掙扎。從精神盈虛的角度看，這可能是你的目標（精）與承載能力（神）之間的不匹配，可能也反映了月令（原生環境）對你的期待與你內在真實需求之間的衝突」
  - 如果遇到「身強無洩」：要說「你的能量很強（神旺），但找不到合適的出口（精衰），就像一個精力充沛的人被困在房間裡，容易內耗、焦慮，需要找到釋放能量的方式（比如運動、創作、工作）。這屬於『神旺精衰』的狀態，能力過剩但目標匱乏，容易內耗空轉。這可能也反映了月令（環境限制）與你內在能量之間的矛盾」
  - 如果遇到「財官過重壓制日主」：要說「你的目標和壓力（精旺）超過了你的承載能力（神弱），就像一個人想要承擔太多責任，但體力或能力跟不上，容易感到壓力過重、身心俱疲。這屬於『精旺神枯』的狀態，目標大但能力不足，容易得而不能守」
  - 如果遇到「地支刑穿」：要說「你的命盤中有XX穿（如卯辰穿、子未穿等），這代表能量流通不順暢，就像身體的經絡不通，容易導致『氣濁神枯』的狀態。這不僅影響你的命運發展，也可能影響你的身心健康。例如卯辰穿可能導致肝木逆、脾胃失調，就像中焦氣滯；子未穿可能導致心腎不交、水火失位，就像神氣無依。需要通過調節生活習慣、情緒管理等方式來改善能量流通」
...
**📌 計算邏輯：** 先完成以下計算，再呈現結果。計算前須先完成：定日主強弱→審月令旺衰→抓命局核心矛盾→定原局用神，以正確解讀五行強弱的喜忌意義。

1. **五行計分：** 天干1分、地支本氣1分、中氣0.5分、餘氣0.3分
2. **月令調整：** 月令為提綱，對日主旺衰影響最大。判斷月令旺衰程度（過旺/偏旺/中和/偏弱/過衰）：是否當令、同黨多寡、是否受沖克合化損傷。需明確標註月令對各五行的生扶或剋洩作用；月令過旺或偏旺→喜克泄耗，過衰或偏弱→喜生扶，月令為忌→優先制化
...
綜合分析方法論
第一步：系統診斷（基礎定位 + 體用做功分析）

**1.1 原局基礎分析（請依序執行）**：
- **定日主強弱**：日干生於何月？得令、得地（地支根氣）、得生（印星）、得助（比劫）與否？綜合判斷身強、身弱或從格。
- **審月令旺衰**：月令為何十神？判斷旺衰程度（過旺/偏旺/中和/偏弱/過衰）：是否當令、同黨多寡與生源、是否受沖克合化損傷、對日主影響是否劇烈。月令過旺或偏旺→喜克泄耗；過衰或偏弱→喜生扶；為忌→優先制化；為喜→優先扶助。
- **抓命局核心矛盾**：最旺/最弱五行？日主與月令關係？刑沖合會？確定主要問題：月令太旺需制化？日主太弱需生扶？兩者交戰需通關？
- **定原局用神**：扶抑（扶日主或制過旺）、調候（寒暖燥濕）、通關（化解相戰）。若原局無直接可用之神，分析大運流年何類五行最有益。
- **結論**：日主強弱、格局類型、核心喜用神與忌神。行運喜忌：喜神=幫身、助用神、制化月令忌神；忌神=加重月令之弊、損用神、加劇矛盾。同一五行因輕重位置不同可有喜忌轉換。

**1.2 定體用（盲派核心）**：這是判斷生命能量是否有效轉化的基礎。必須明確回答：
- **日主是什麼？**（用白話說明：你出生那天的能量特質）
- **命局中什麼代表「神（體）」？**（日主及印比，代表承載目標的能力、心智與能量）
- **命局中什麼代表「精（用）」？**（財官，代表欲望、目標、事業與現實成就）
- **原局是否「有精可用」？是否「有神可承」？**（用生活化比喻說明：你的目標是否明確？你的能力是否足以承擔目標？）

**1.3 找原局卡點（關鍵診斷）**：找出原命局最主要的氣機問題（只選重點，不可堆砌）：
- ☐ **庫多封閉**：能量被封存，無法流通（用比喻說明：就像錢存在銀行但取不出來）
- ☐ **刑穿阻滯**：地支刑穿導致能量流通不順（用比喻說明：就像身體經絡不通）
- ☐ **神旺精衰**：能力過剩但目標匱乏（用比喻說明：就像體力充沛但不知道要做什麼）
- ☐ **精旺神枯**：目標很大但能力不足（用比喻說明：就像想要很多東西但體力跟不上）
- ☐ **水火不交**：陰陽能量無法調和（用比喻說明：就像冷熱無法平衡）
- ☐ **用神被困**：對你有利的能量被壓制（用比喻說明：就像你的幫手被綁住了）
- ☐ **其他**（需具體說明）

**必須用一句話說明**：此命目前「功做不起來」的真正原因是什麼？（用生活化語言，不要只說術語）

**1.4 大運定位**：確定當前大運干支的喜忌屬性，建立十年運勢基準線。

**1.5 流年定調**：分析流年干支在原局與大運背景下的戰略意義。

第二步：能量解碼（三層互動 + 做功本質分析）

**2.1 判大運的本質作用（關鍵）**：不要只看大運的喜忌，要看大運如何影響原局的氣機流通。

必須回答：
- **此大運主要動到的是體（神）還是用（精）？**（用白話說明：這步大運主要影響的是你的能力基礎，還是你的目標和壓力？）
- **它是在：**
  - ☐ **開庫**：打開被封存的能量（用比喻說明：就像打開了儲藏室）
  - ☐ **引通**：引導能量流通（用比喻說明：就像疏通了管道）
  - ☐ **化解**：化解原局的矛盾（用比喻說明：就像調解了衝突）
  - ☐ **疊加**：只是增加力量（用比喻說明：就像只是加量不加質）
  - ☐ **封鎖**：進一步封閉能量（用比喻說明：就像把門鎖得更緊）
- **它是否改善原局卡點，還是只是增加力量？**（必須明確判斷：是結構性改善，還是只是數量上的增減）

**結論用一句話**：這步大運屬於【真做功運／假旺運／耗神運／結構改善運】之一。（用生活化語言解釋：真做功運=有效轉化能量；假旺運=表面風光但無實質；耗神運=消耗能力但無成果；結構改善運=改善原局卡點）

**2.2 判流年的實際功能（關鍵）**：不要只看流年的喜忌，要看流年如何與大運互動，產生實際效果。

必須回答：
- **此流年是：**
  - ☐ **幫大運落實**：協助大運的能量轉化為實際成果（用比喻說明：就像給大運的計劃提供了執行力）
  - ☐ **測試大運**：檢驗大運的能量是否真實有效（用比喻說明：就像考試檢驗學習成果）
  - ☐ **消耗大運**：消耗大運的能量但無實際產出（用比喻說明：就像消耗了資源但沒有收穫）
- **流年是否：讓氣機更流動？還是讓壓力集中爆發？**（用生活化場景說明：是疏通還是堵塞？）

**用一句話總結**：此流年是【結果年／壓力年／過渡年／破局年】。（用生活化語言解釋：結果年=能量轉化為成果；壓力年=壓力集中但無出口；過渡年=能量轉換期；破局年=打破原有結構）

**2.3 大運流年關係矩陣**（結合上述本質分析）：

大運喜用 + 流年喜用 = 順勢而上（強化機遇）→ 但需判斷：是真做功還是假旺？
大運喜用 + 流年忌神 = 逆風前行（壓力中成長）→ 但需判斷：壓力是否轉化為動力？
大運忌神 + 流年喜用 = 暗夜明燈（困境中突圍）→ 但需判斷：是否真的突圍還是只是暫時緩解？
大運忌神 + 流年忌神 = 韜光養晦（防守為主）→ 但需判斷：是防守還是被動消耗？
刑沖合害分析重點：
...
**體用定位**：
- 神（體）：[日主及印比，用白話說明其特質與強弱]
- 精（用）：[財官，用白話說明其特質與強弱]
- 精神匹配度：[用一句話說明：是精神俱足/精旺神枯/神旺精衰/氣濁神枯，用生活化比喻]

核心喜用：[五行]，核心忌神：[五行]

關鍵矛盾：[如：比劫奪財，需官殺制比護財]

**原局卡點**：[用一句話說明此命目前「功做不起來」的真正原因，用生活化語言，不可堆砌]
...
【第一步｜定體用】
- 日主是什麼？[用白話說明]
- 命局中什麼代表「神（體）」？[日主及印比，用白話說明其特質]
- 命局中什麼代表「精（用）」？[財官，用白話說明其特質]
- 原局是否「有精可用」？是否「有神可承」？[用生活化比喻說明]

【第二步｜找原局卡點】
- 原命局最主要的氣機問題：[從庫多封閉/刑穿阻滯/神旺精衰/精旺神枯/水火不交/用神被困/其他中選擇，並用一句話說明此命目前「功做不起來」的真正原因，用生活化語言]

【第三步｜判大運的本質作用】
- 此大運主要動到的是體（神）還是用（精）？[用白話說明]
- 它是在：[開庫/引通/化解/疊加/封鎖]（用比喻說明）
- 它是否改善原局卡點，還是只是增加力量？[明確判斷]
- **結論**：這步大運屬於【真做功運／假旺運／耗神運／結構改善運】之一（用一句話說明，用生活化語言解釋）

【第四步｜判流年的實際功能】
- 此流年是：[幫大運落實/測試大運/消耗大運]（用比喻說明）
- 流年是否：讓氣機更流動？還是讓壓力集中爆發？[用生活化場景說明]
- **結論**：此流年是【結果年／壓力年／過渡年／破局年】（用一句話總結，用生活化語言解釋）

【第五步｜做功 vs 逼功判斷（關鍵）】
- 是否形成「體能承用、用能反哺體」的循環？[用生活化場景說明：就像你的能力能承擔目標，目標達成後又增強了你的能力]
- 還是出現「事情先來、責任先到、資源未至」？[用生活化場景說明：就像任務來了但沒有足夠的資源和準備]
- **✅ 結論只能二選一**：
  - ☐ **做功（有效轉化）**：能量有效轉化為成果，形成良性循環（用一句話說明具體表現）
  - ☐ **逼功（硬撐消耗）**：表面在做事但實際在消耗，無法形成有效轉化（用一句話說明具體表現）

【第六步｜喜用結論（不得模糊）】
**判斷結論**：

此【大運／流年】對命主屬於：
- ☐ **喜**：打通氣機並產生實際成果（用一句話說明具體理由）
- ☐ **平**：無明顯改善也無明顯惡化（用一句話說明具體理由）
- ☐ **忌**：加劇原局卡點或消耗能量（用一句話說明具體理由）

**理由只限一句話**：（說明它是否「打通氣機並產生實際成果」，用生活化語言）
...
**最終鐵則（必須內化）**：
喜用不是讓人更強，而是讓命局原本卡住的地方開始流動。真正的好運，是能量能夠有效轉化為成果，而不是單純的力量增加。

流年地支深度解構

**四柱管運參考（綜合參斷）：**
- **年柱管少年運(1-18歲)**：流年與年支的互動，主要影響祖輩、根基、外在形象、早年環境
- **月柱管青年運(19-36歲)**：流年與月支的互動，主要影響人際關係、工作環境、兄弟姐妹、青年發展
- **日柱管中年運(37-54歲)**：流年與日支的互動，主要影響婚姻、合作、自身狀態、中年運勢
- **時柱管晚年運(55歲後)**：流年與時支的互動，主要影響子女、下屬、晚年運勢、傳承

**注意：** 以上年齡劃分為參考，實際分析時需結合命主當前年齡與大運階段，綜合參斷各柱的影響力。
...
**能量狀態**：[基於三方互動判斷，需結合體用做功分析，說明是「做功」還是「逼功」狀態]

**可能事象**：[列舉2-3種最可能情況，按概率排序，需說明這些事象是能量有效轉化的結果，還是硬撐消耗的表現]

**做功效率評估**：[明確說明此領域的能量是否有效轉化為成果，還是只是表面忙碌但無實質產出]

**應對策略**：[積極策略/防守策略，需基於做功 vs 逼功的判斷，如果是逼功狀態，建議調整策略避免硬撐消耗]
...
**前置：核心分析步驟（六親分析前須先完成）**：定日主強弱（得令得地得生得助）→ 審月令旺衰（月令十神、旺衰程度：過旺/偏旺/中和/偏弱/過衰，圍繞月令取用神）→ 抓命局核心矛盾（最旺/最弱五行、日主與月令關係）→ 定原局用神與喜忌（扶抑、調候、通關；行運喜忌動態分析）。此為「以日主喜忌為綱」判斷六親的基礎。
...
### 步驟 1.5：藏干判斷（關鍵細節，不可忽略）

**核心原則**：論六親時，必須同時看天干透出和地支藏干，藏干是分析六親時不可忽略的關鍵細節。

**藏干的重要性**：
- 天干透出為**主星**（表象、行為模式、外在特質）
- 地支藏干為**輔星/根氣**（根基、內在特質、隱性資源）
- 兩者共同構成六親星的完整能量結構
...
**① 主星與輔星的區分**：
- **主星**：天干透出的十神（如月干透偏財庚金）
- **輔星/根氣**：地支藏干中的同類十神（如年支丑中藏正財辛金）
- 兩者需同時納入分析，不可只論透出而忽略藏干

**② 正偏財/正偏印的組合判斷**：
- **父親**：偏財透出+正財藏干 → 主星為偏財，輔星為正財，構成父親星能量的雙重性
- **母親**：正印透出+偏印藏干 → 主星為正印，輔星為偏印，構成母親星能量的雙重性
- 無偏財時看正財，無正印時看偏印，但需同時看透出與藏干
...
**⑤ 實際應用範例**：
- 例：月干透偏財庚金（主星），年支丑中藏正財辛金（輔星/根氣）
- 判斷：父親星由偏財（表象）和正財（根基）共同構成
- 分析：需同時論述兩者的喜忌、旺衰、對命主的影響

**⚠️ 關鍵提醒**：藏干必須納入計算，這是分析六親時不可忽略的關鍵細節。忽略藏干會導致分析不完整、論斷不準確。
...
**六親星完整構成判斷**：
- 需同時標註：主星（天干透出）+ 輔星/根氣（地支藏干）
- 例：父親星 = 偏財庚金（月干透出，主星）+ 正財辛金（年支丑藏，輔星/根氣）
- 兩者共同構成該六親的完整能量結構

**影響力判斷**：越旺+越近日主→影響越大；喜用→助力，忌神→拖累。

**操作準則**：
- ✅ **必須**：藏干必須納入計算，這是分析六親時不可忽略的關鍵細節
- ✅ **必須**：同時看透出與藏干，給出完整的六親星構成
- ✅ **必須**：區分主星（表象）與輔星/根氣（根基）的不同作用
- ⚠️ **禁止**：只論透出而忽略藏干
- ⚠️ **禁止**：低顯著度≠不存在，藏干即使不顯也要納入分析
...
**總綱**：以年月柱為父母宮，結合財星（父）、印星（母）的喜忌與旺衰，綜結論斷其品質、條件及對命主的助益。

**核心法則**：

**① 宮星結合**：
- 先觀年月柱干支之喜忌，定父母宮氣數
- 再專看財星（父）、印星（母）之為喜為忌、有力無力

**② 星之定位**（必須同時看透出與藏干）：
- **父**：以偏財為主，正財兼看；無偏財則以正財論
  - **必須同時看**：天干透出的財星（主星）+ 地支藏干中的財星（輔星/根氣）
  - 例：偏財庚金透月干（主星）+ 正財辛金藏年支丑（輔星/根氣）→ 父親星由兩者共同構成
  - 需同時論述主星與輔星的喜忌、旺衰、對命主的影響
- **母**：以正印為主，偏印兼看；無正印則以偏印論
  - **必須同時看**：天干透出的印星（主星）+ 地支藏干中的印星（輔星/根氣）
  - 例：正印透出（主星）+ 偏印藏干（輔星/根氣）→ 母親星由兩者共同構成
  - 需同時論述主星與輔星的喜忌、旺衰、對命主的影響
- （註：實踐中常可通論，不必過分拘泥正偏，尤以年月見財即與父關聯）
- **⚠️ 關鍵提醒**：藏干是分析父母時不可忽略的關鍵細節，必須納入計算

**③ 喜忌論斷（四種組合）**：
- **宮星皆喜**：父母品質條件佳，且對命主助益大
- **宮星皆忌**：父母品質條件差，且對命主無助益，甚或拖累
- **宮忌星喜**：父母自身條件或品質一般，但對命主確有實質助益
- **宮喜星忌**：父母條件不錯，但對命主助益有限或方式不佳

**④ 星之專論**：
- **財星為父**：務要生旺有力為喜，主父親有作為，命主得父蔭。為忌則無助，若再壞印（如財旺剋印），可主父親敗業或帶來麻煩
- **印星為母**：生旺為喜，主母親賢慧慈愛，命主得母蔭。為忌則無助，過旺為忌更主母親干預過多或形成拖累

**特殊情形與細節推斷**：

**① 星曜逢合**：
- 財星（尤偏財）在年月逢合，主父親外緣佳、心思外向
- 印星在年月逢合，主母親外緣佳、心思外向。印星合多，更暗示母親易有外情

**② 神煞與坐下**：
- 印星坐桃花（如沐浴）、或合多，可主母親情慾較重、風流
- 以印星坐下十二運斷母性：如坐長生為喜，主母親長壽慈惠；坐沐浴，主母親多情；坐絕地，主母親緣薄或體弱

**③ 星曜受剋與配置**：
- 正印明現而受旺財所剋，又見傷官多，可論母親情感複雜、多情多欲（原理：傷官生財，財再生殺，對正印形成情感與壓力的雙重剋擾）
- 命局中印星或財星最為旺強，則該親對命主影響力最大，其喜忌吉凶效應亦倍增

**活用與延伸**：
- 此框架可「舉一反三」：論六親（如祖輩看年柱，配偶看日支等）皆可沿用「宮位為基，用星為憑，喜忌為斷，旺衰為量」的核心邏輯
- 需結合全盤氣勢，不可執一。如從格、化氣格等特殊格局，則需依從格之理論斷六親
- 「品質」包含德行、能力、社會地位；「條件」指經濟、環境資源；「助益」指實際對命主產生的生扶、庇護等正面作用

**總結心法**：
**「宮位定基調，星曜辨親緣。喜忌分優劣，旺衰量力源。合沖察外意，坐支看內性。生剋組合論，全局須通變。」**
...
**⚠️ 重要要求**：你必須充分利用上述所有分析邏輯，輸出詳細、深入、實用的分析內容。每個部分都要有具體的論斷和解釋，避免空泛或過於簡略。
...
**② 六親十神定位總表**（必須包含所有六親，必須包含藏干）：
以表格形式呈現，每行包含：
- 六親角色（父/母/兄弟/配偶/子女）
- 對應十神（需標註正偏，必須同時標註主星和輔星/根氣）
  - 主星：天干透出的十神及位置
  - 輔星/根氣：地支藏干中的同類十神及位置
  - 例：父親 = 偏財庚金（月干，主星）+ 正財辛金（年支丑藏，輔星/根氣）
- 顯著度（🔴高/🟡中/🟢低，需說明判斷依據，必須包含藏干計算）
- 所在宮位（年/月/日/時柱，具體位置，需標註透出與藏干位置）
- 喜忌狀態（喜用/忌神，需分別說明主星與輔星的喜忌及對命主的影響）
- 得位/飛宮/游離狀態
- 元氣評級（🔴弱-耗心力/🟡中-平穩/🟢強-較安心）
- 刑沖合害情況（如有）

**③ 系統能量總評**：
- 整體家庭能量模式（哪些關係是助力，哪些是壓力源）
- 核心關係議題（2-3個重點）
- 家族能量流動特徵

### 2. 成員深度解析（必須詳細，至少分析3-4個重點成員）

**要求**：每個成員的分析必須包含以下所有要素，不得省略：
...
- **星之定位**（必須同時看透出與藏干）：
  - 主星：天干透出的十神（表象、行為模式）
  - 輔星/根氣：地支藏干中的同類十神（根基、內在特質）
  - 需明確標註該六親星的完整構成（主星+輔星/根氣）
  - 例：父親星 = 偏財庚金（月干透出，主星）+ 正財辛金（年支丑藏，輔星/根氣）
  - 需同時論述主星與輔星的喜忌、旺衰、對命主的影響
- **宮位分析**：所在宮位的喜忌，與命主的關係遠近
- **得位/飛宮/游離判斷**：具體說明是得位、飛宮還是游離，並解釋其含義
- **喜忌判斷**：該六親星為喜用還是忌神，對命主的實際影響
- **太極反斷**：以該六親為太極點，分析其在原局中的處境（如適用）
- **位正判斷**：是否品行端正、角色分明
- **庇蔭分析**：該六親對命主是否有實際助益，如何體現
...
**③ 歲運變化詳解**：
- 結合當前大運，說明該六親在此階段的實際作用
- 預測未來大運流年中的變化趨勢
- 不同時期該六親對命主的影響變化

**④ 命主感受推測**：
- 基於能量模式，推測命主在此關係中可能產生的內在感受
- 可能的心理壓力或情感需求
- 關係中的能量互動模式（使用「可能」「傾向」等表述）

**⑤ 具體論斷**：
- 該六親的品質、條件、能力特徵（基於星曜旺衰、喜忌、配置）
- 對命主的實際助益或拖累程度
- 關係中的具體表現和互動模式

**特別要求 - 父母專論**（如分析父母）：
- 必須運用「父母品質與助益專論」的所有法則
- **必須同時看透出與藏干**：明確標註主星（天干透出）和輔星/根氣（地支藏干）
- 詳細分析宮星結合、四種喜忌組合、星曜逢合、神煞坐下等
- 需同時論述主星與輔星的喜忌、旺衰、對命主的影響
- 給出具體的品質、條件、助益論斷（基於完整的星曜構成）
...
- ❌ 過於簡略或空泛
- ❌ 只列框架不給內容
- ❌ 遺漏重要的分析步驟（須依序完成：定日主強弱→審月令旺衰含旺衰程度判斷→抓命局核心矛盾→定原局用神→行運喜忌）
- ❌ 使用絕對化表述（需用「可能」「傾向」等）
...
1. **定日主強弱**：日干生於何月？得令、得地（根氣）、得生（印）、得助（比劫）與否？綜合判斷身強、身弱或從格。

2. **審月令旺衰**：月令為何十神？判斷旺衰程度（過旺/偏旺/中和/偏弱/過衰）：是否當令、同黨多寡與生源、是否受沖克合化損傷、對日主影響是否劇烈。月令過旺或偏旺→喜克泄耗；過衰或偏弱→喜生扶；為忌→優先制化；為喜→優先扶助。

3. **抓命局核心矛盾**：最旺/最弱五行？日主與月令關係？刑沖合會？確定主要問題：月令太旺需制化？日主太弱需生扶？兩者交戰需通關？

4. **定原局用神與格局**：扶抑、調候、通關。確定格局類型（正格/變格）、喜用神與忌神。行運喜忌：喜神=幫身、助用神、制化月令忌神；忌神=加重月令之弊、損用神。同一五行因輕重位置可有喜忌轉換。

5. **核心矛盾識別**：找出命局中最強烈的能量衝突（如：身弱殺旺、財多身弱、比劫奪財等），並分析這種衝突在現實中的具體表現。

6. **能力值轉化：** 將十神強弱轉化為六維能力值時，需考慮：
   - 該十神在原局中的強弱（得令、得地、得生、得助）
   - 該十神是否為喜用神（喜用則加分，忌神則減分）
   - 該十神是否受剋制（受剋則能力受限）

7. **人物鏡像選擇：** 根據命主的性格特質（十神組合）和人生軌跡（大運流年），選擇最貼切的人物鏡像。需說明「同步率」的具體原因，而非簡單套用。

8. **矛盾信息處理：** 若命盤顯示某種特質，但現實表現相反（如：身弱但實際很強勢），需深入分析：
   - 大運流年是否暫時改變了原局狀態
   - 是否有其他十神組合抵消了弱點
   - 是否通過後天努力轉化了先天配置
...
**📌 分析邏輯：** 先完成以下分析，再呈現結果。**前置**：定日主強弱→審月令旺衰（含旺衰程度：過旺/偏旺/中和/偏弱/過衰）→抓命局核心矛盾→定原局用神。健康分析雖「不看喜用」，但五行強弱與月令旺衰是判斷失衡的基礎。

1. **五行強弱計算：** 計算各五行在原局中的強弱（包含藏干），並標註月令旺衰程度
...
### 🎓 1. 學習結構總覽（基於喜忌重審）
- **前置分析**：定日主強弱→審月令旺衰（含旺衰程度）→抓命局核心矛盾→定原局用神→行運喜忌。
- 明確日主強弱與核心喜忌。  
...
**前置：核心分析步驟（職涯分析前須先完成）**：定日主強弱（得令得地得生得助）→ 審月令旺衰（月令十神、旺衰程度：過旺/偏旺/中和/偏弱/過衰）→ 抓命局核心矛盾 → 定原局用神與喜忌。此為判斷「適合發揮該角色的產業環境」與「喜用五行方向」的基礎。
...
在分析工作角色時，「十神」能更具體地反映一個人的行為動機與能量輸出模式：

- **印星（正印、偏印）**：屬於思考型。重視資源、邏輯與知識，適合幕僚或研究崗位。在職場中傾向於深度學習、系統整理與長期規劃，是團隊中的知識支撐者。

- **官殺（正官、七殺）**：屬於執行與管理型。正官守禮守節，適合在穩定體系中建立秩序；七殺具侵略性與冒險精神，適合在競爭激烈的領域開疆闢土。兩者都傾向承擔責任與決策，但風格不同。

- **食傷（食神、傷官）**：屬於創意型。擅長表達、設計與傳播，是點子最多的隊友。在職場中傾向於將想法轉化為產出，但需注意區分「靈感表達」與「可落地成果」。

- **財星（正財、偏財）**：屬於管理與業務型。對數字敏感，追求實質成果，適合銷售或資產管理。在職場中高度關注資源轉換與效益回報，擅長在條件限制中做取捨。

- **比劫（比肩、劫財）**：屬於社交與連結型。重視團隊精神，但也可能較具競爭心。在職場中傾向自行主導節奏、直接行動，對自主權與決定權有高度需求。

**實戰識人建議（團隊協作與風險管理）：**

在實際職場應用中，除了分析個人命盤，也可以運用十神與喜忌的邏輯來評估人際互動：

- **招募與合夥**：觀察對方的「旺神」是否為你的「喜神」。若對方強項能補足你的短處（如你缺水、對方水旺），則是強大的助力。例如，你身弱需要印星支撐，而對方印星強旺，則可能在知識、資源或精神支持上形成互補。

- **防範小人**：若對方的旺神恰好是你的「忌神」，容易在氣場上產生壓制或衝突，導致溝通疲憊或事倍功半。例如，你忌官殺（壓力），而對方官殺極旺，可能在合作中感受到過度壓力或規則束縛。

- **團隊協作**：高壓環境（如新創公司）適合多用「七殺」重的人，他們能在競爭中快速推進；穩定體系（如大型企業）則適合「正官」或「正印」強的員工，他們更能在規則框架中穩步成長。

**注意**：以上識人建議應作為輔助參考，不可過度依賴。實際工作中仍需以具體表現、溝通能力與價值觀匹配為主要判斷依據。
...
**📌 在輸出意象之前，請先在內部依序完成：**
1. **定日主強弱**：得令、得地、得生、得助，判斷身強/身弱
2. **審月令旺衰**：月令十神、旺衰程度（過旺/偏旺/中和/偏弱/過衰）
3. **抓命局核心矛盾**：最旺/最弱五行、日主與月令關係
4. **定原局用神與喜忌**：扶抑、調候、通關
5. 當前大運與原局的互動關係、流年與大運的疊加效應
...
**前置：核心分析步驟（情感分析前須先完成）**：定日主強弱（得令得地得生得助）→ 審月令旺衰（月令十神、旺衰程度：過旺/偏旺/中和/偏弱/過衰，圍繞月令取用神）→ 抓命局核心矛盾（最旺/最弱五行、日主與月令關係）→ 定原局用神與喜忌（扶抑、調候、通關；行運喜忌動態分析）。
...
**📌 轉化邏輯：** 先完成以下分析，再轉化為遊戲屬性。**前置**：定日主強弱→審月令旺衰（含旺衰程度）→抓命局核心矛盾→定原局用神與喜忌。

1. **日主強弱判定：** 判斷日主旺衰（得令、得地、得生、得助），轉化為 HP 值
2. **格局類型識別：** 根據格局類型（正格/變格）與十神組合，確定職業定位（如：身強殺旺=輸出型、身弱印旺=輔助型、食傷旺=創意型等，用現代職場角色比喻）
