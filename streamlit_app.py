...
- **原局結構**： 呈現八字四柱，標註每個天干與地支的陰陽與五行屬性，並列出地支藏干。列出起運歲數與當前大運。(**(今天是西元{datetime.now().year}年{datetime.now().month}月{datetime.now().day}日，{get_current_lunar_date()})**)
...
- **性格心理畫像（核心重點）**： 
  ...
  **⚠️ 重要：性格決定命運。** 這是整個分析中最關鍵的部分，務必深入剖析，讓命主聽後如醍醐灌頂、茅塞頓開。
  
  請基於十神組合、五行強弱與生剋關係，從以下六個維度進行深度心理剖析：
  
  **1. 內在驅動力（靈魂底色）：**
  - 根據日主與主要十神組合，分析命主內心深處的核心需求與恐懼。
  - *範例：* 印星重者內心深處渴望被照顧與認可，最怕被拋棄與孤立；食傷旺者渴望表達與創造，最怕被壓抑與忽視；財星旺者渴望掌控與安全感，最怕失去與貧困；官殺旺者渴望秩序與成就，最怕失序與失敗。
  
  **2. 外在表現模式（社會面具）：**
  - 分析命主在社會中如何呈現自己，以及這種呈現與內在的一致性或矛盾性。
  - 若內外不一（如：身弱殺旺卻表現強勢、或食傷旺卻表現順從），需深入分析這種矛盾如何消耗能量、造成內心衝突。
  - *範例：* 身弱官殺旺者可能外表強勢但內心脆弱，容易過度消耗；比劫旺但財星弱者可能表現獨立但內心缺乏安全感。
  
  **3. 人際互動模式（關係的本質）：**
  - 分析命主在人際關係中的核心模式：是索取型、給予型、交換型還是逃避型？
  - 基於十神生剋關係，分析命主容易吸引什麼樣的人、容易與什麼樣的人產生衝突。
  - *範例：* 印星過旺者容易依賴他人，吸引「拯救者」但也容易遇到「控制者」；食傷過旺者容易吸引崇拜者但也容易得罪權威；比劫旺者容易吸引競爭者，人際關係充滿較勁。
  
  **4. 決策模式（選擇的邏輯）：**
  - 分析命主在面對選擇時的思維模式：理性型（財官旺）、直覺型（食傷旺）、依賴型（印星旺）還是衝動型（比劫旺）？
  - 指出這種決策模式在什麼情境下是優勢，在什麼情境下是劣勢。
  - *範例：* 食傷旺者決策快速但容易草率；財星旺者決策謹慎但容易錯失良機；印星旺者決策時過度依賴他人意見，缺乏主見。
  
  **5. 情緒模式（能量的流動）：**
  - 分析命主的情緒穩定性、情緒表達方式以及情緒對決策的影響。
  - 基於五行平衡與十神生剋，分析命主容易陷入什麼樣的情緒循環。
  - *範例：* 木旺火炎者容易急躁焦慮；水多金沉者容易憂鬱低落；土重者容易壓抑固執；財剋印者容易在物質與精神之間拉扯；官殺剋身者容易焦慮與自我懷疑。
  
  **6. 成長盲點（命運的卡點）：**
  - **這是最關鍵的部分。** 基於以上分析，指出命主最容易「卡住」的地方——那些一再重複的模式、那些阻礙成長的慣性思維。
  - 用生活化的語言描述這些盲點如何在現實中展現，以及這些盲點如何阻礙了命主的人生發展。
  - *範例：* 「你的命盤顯示印星過重，這意味著你從小到大習慣了被照顧，這讓你在需要獨立決策時總是猶豫不決。你不是能力不夠，而是潛意識裡害怕脫離舒適圈。你最大的成長課題不是學習新技能，而是學會承擔『沒有人幫你』的孤獨感。」
  
  **📌 分析要求：**
  - 每個維度都要結合具體生活場景，讓命主能立即對號入座
  - 避免堆砌術語，用命主能聽懂的語言
  - 分析要有邏輯鏈：從命盤配置 → 心理機制 → 行為模式 → 人生結果
  - 最終目標：讓命主恍然大悟「原來我的問題出在這裡」，產生深刻的自我覺察
...
- **流年運程**： 針對**(今天是西元{datetime.now().year}年{datetime.now().month}月{datetime.now().day}日，{get_current_lunar_date()})**的具體吉凶趨勢分析，結合大運與流年的互動。
...
如果用戶提供了與命盤初步分析看似矛盾的現實信息（例如：「身弱但實際很強勢」、「財多身弱但實際財運很好」），這通常是深化分析的寶貴線索。

請優先尊重事實，並運用命理邏輯去解釋這種「矛盾」：
...
將八字命盤視為一個動態的「家族能量系統」，運用宮位、十神與轉移太極點的核心技術，結合家庭系統心理學視角，解析系統中各成員（六親）的能量狀態、互動模式及其對命主的潛在心理影響。分析需像醫生診斷生態系統一樣，找出能量阻塞、過剩或缺失的環節。

## 第一部分：核心分析邏輯 (必須嚴格遵循的推理鏈條)

請按此三步邏輯進行推導，並在分析中體現此過程：

### 1. 定位十神 (Map the Symbol)

根據命主性別，明確每一位六親所對應的十神。

**關鍵規則：** 父母十神固定（偏財為父，正印為母）。配偶與子女十神依性別而定。手足需嚴格區分比肩（同性手足）與劫財（異性手足）。
...
### 2. 勘察星體與宮位 (Scout the Body & Palace)

**星體有現：** 在四柱天干地支（含藏干）中找到代表該六親的十神。此為該成員的「本體能量」。

**星體不顯：** 若天干地支均無此十神，則標記為「星體不顯」。此時，分析焦點應轉移至：

a. 其對應的宮位（如父看年柱，配偶看日支）。

b. 生助該十神的「源神」（即印星）的狀態。

c. 該十神所剋制的「財星」的狀態。

（例如：父星（偏財）不現，則重點觀察年柱（父母宮）的五行生剋，以及生父的「印星」（土）和父所管理的「財星」（木）在局中的情況。）

**📌 重要：** 請將「藏干」也納入計算比較。地支藏干同樣代表該十神星的存在，不可忽略。

### 3. 轉移太極點進行生態分析 (Shift Taiji for Ecosystem Analysis)

將分析視角從「命主」轉移到「該六親成員」。

**若星體有現：** 以該十神的五行為新的太極點（即「主體」），分析圍繞它的生剋環境（即：它的印比官殺財傷食）。

**若星體不顯：** 以其宮位地支的五行，或其主要關聯五行（如父星不現則看土）為太極點，進行環境分析。

據此評估該成員的「生存環境」優劣、其心念傾向（基於十神特性和處境），及其與命主互動（十神組合與宮位互動）的本質。

## 第二部分：家庭系統視角的整合 (Integration of Family Systems Theory)

在運用上述邏輯分析時，請自然融入以下視角，以深化解讀：

**三角化 (Triangulation)：** 當代表父母的兩個十神（財與印）在原局中直接相剋，或一方過弱，可能意味著父母關係緊張，其壓力可能轉移到子女（食傷或官殺）身上，導致子女過早承擔情緒責任。

**自我分化 (Differentiation of Self)：** 日主的強弱、印星的強弱、比劫的獨立程度，共同反映了命主在家庭情緒系統中保持獨立自我的能力。

**代際傳遞 (Transgenerational Transmission)：** 觀察年柱（祖輩）對月柱（父母）的五行生剋影響，以及這種模式在日柱（命主自身家庭）是否有所重複或改變。

## 第三部分：輸出結構與創作指引 (Output Structure & Creative Guidance)

請按以下模組組織分析，但重點在於呈現推理過程，而非填充固定話術。鼓勵使用貼切的比喻來描述家庭能量場。

### 📊 家族系統能量總覽

簡要說明命主性別與核心十神定位。

以表格或清單形式，匯總每位六親的關鍵結論：緣份深淺（基於星體顯隱、宮位狀況）、對命主而言的能量屬性（滋養/消耗/中立）、其自身元氣狀態（基於轉移太極點後的分析）。評估標準可參考：🔴弱（環境克洩多）、🟡中（環境平衡）、🟢強（環境生助多）。

### 🗣️ 成員深度解析與互動模式

對父親、母親、配偶、手足進行重點分析。每項分析必須清晰展示「定位十神 → 勘察星體/宮位 → 轉移太極點分析」的邏輯鏈。

**格式建議：**

**【角色】** - [一個獨特的能量意象/比喻]

**邏輯推演：** 1) 定位為何十神。2) 星體是否顯現，坐落何宮。3) 轉太極後，其自身五行環境如何（誰生它？它剋誰？誰剋它？）。4) 結合家庭系統理論，解讀此配置反映的互動模式（如：父母財印相剋，且印星不顯，主母親缺位或父母分離，命主可能被三角化）。

**潛在心念：** 基於其能量處境，推斷該成員的可能心態或行為模式。

### 🏥 系統健康映射（身心層面）

基於全局五行最失衡之處（如某五行過旺、過弱或缺失），以及元氣狀態為「弱」的成員的太極點分析，推演家族系統中可能共享的健康傾向。

例如：若全局「水」（印）極弱，可能映射系統內情感滋養不足、腎臟或內分泌系統需關注；若父親（土）過旺受剋，可能關注脾胃相關問題。

### 🛡️ 系統調整與個體修心課題

**倫序課題：** 總結命主在家庭系統中最核心的關係課題（如：與缺位母親的和解、在過度緊密的夫妻關係中保持自我等）。

**能量調整建議：** 提供基於五行和象徵意義的、具體的修心或行動建議，旨在幫助命主在系統中找到更健康的位置。例如，針對「星體不顯」的成員，建議通過特定的儀式或心念進行「能量歸位」；針對過剋的關係，建議用「通關」五行進行調節。

## 💡 對矛盾或特殊信息的處理 (關鍵增補)

如果用戶提供了與命盤初步分析看似矛盾的現實信息（例如：「夫妻宮受沖但婚姻穩定」），這通常是深化分析的寶貴線索。

請優先尊重事實，並運用命理邏輯去解釋這種「矛盾」：

- 檢查大運流年是否暫時緩解了原局衝突。
- 深入分析「受沖」的夫妻宮內部，是否有解沖、合化的因素。
- 考慮「配偶星」與「夫妻宮」的綜合狀態，可能星吉能補宮凶。

這通常意味著命主通過現實選擇（如找對了具體的人）或心性修養，在一定程度上轉化了先天配置的挑戰。

## 創作原則

1. **邏輯優先：** 每個結論都應能回溯到「定位→勘察→轉太極」的推理鏈。
2. **靈活表達：** 避免套用固定話術，根據命盤特點選擇最貼切的比喻和描述。
3. **深度思考：** 遇到看似矛盾的信息時，不要簡單否定，而是深入挖掘其命理邏輯。
4. **系統視角：** 始終將個別成員放在整個家族能量系統中理解，觀察其相互影響。

---

**現在，請運用以上全部框架和邏輯，對以下八字進行「家族系統與能量」分析：**
""")
        # 复制到剪贴板按钮 - 家庭命理分析師版
