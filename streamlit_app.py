...
```
  能量值 |
   100  |                [登頂]
    ...
    20  | [啟程]                          \\__ [休養]
      __|__________________________________________
 年齡:    20s    30s    40s    50s    60s    70s
 階段:   啟程   成長   登頂   轉換   守成   休養
```
...
2. **標註關鍵節點**：
   - **啟程**：正式進入學習階段，開始學業旅程（通常為第一個大運或第二個大運）
   - **登頂**：學業能量最高點，學習效率與成果達到巔峰（能量值達到峰值的大運）
   - **轉換**：學習階段結束或轉換方向，從學業轉向實踐應用（能量值明顯下降，或轉向其他領域）
   - **休養**：學習能量進入低潮期，適合沉澱與反思

3. **繪製趨勢線**：
   - 使用平滑曲線連接各點，呈現整體學習能量趨勢
   - 標註每個大運的具體年齡段（如 20s、30s）
   - 在「階段」行標註每個時期的學習特徵（啟程、成長、登頂、轉換、守成、休養）
...
### (B) 未來十年流年學習能量趨勢 (Micro Trend {datetime.now().year}-{datetime.now().year + 9})
根據流年吉凶繪製，需標註年份與學習建議
  ...
  能量值 |
   100  |            [最佳時機]
    ...
    20  |   \\/              (調整期)
        |  2025
      __|______________________________________
 年份:   '25  '26  '27  '28  '29  '30  '31
 建議:   觀察 投入 維持 完成 觀察 調整 回升
```
...
2. **標註關鍵年份與學習建議**：
   - **投入**：適合投入學習、考試、進修，能量充沛（機會值 > 70）
   - **維持**：維持現有學習節奏，穩步前進（機會值 50-70）
   - **完成**：適合完成學業、畢業、轉換階段（機會值高但轉換期）
   - **觀察**：學習能量平穩，無明顯變化，保持現狀（機會值 40-60）
   - **調整**：學習壓力大，需調整策略或放慢節奏（機會值 < 40，且為忌神）
   - **回升**：學習能量回升，適合重新投入或加強學習（從低點回升）

3. **繪製基準線**：以 60 分為基準線，標註高於/低於基準線的年份
   - 高於基準線：學習能量較強，適合積極投入
   - 低於基準線：學習能量較弱，需謹慎規劃或調整策略

4. **標註關鍵節點**：
   - 在圖表上標註具體年份（西元年份，如 2025、2026）
   - 標註「最佳時機」（學業機會值最高的年份，適合重要考試或進修）
   - 標註「調整期」（學習壓力大或結構性衝突的年份，需放慢節奏）
