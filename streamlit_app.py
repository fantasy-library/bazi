...
- **吸引力分析（基於能量疲勞互補模型）**：
  - **你散發的能量**：你在日常中最自然散發出的能量是什麼？（例如：食傷女的「情感表達與共感」）
  - **最易吸引的對象類型**：根據能量疲勞互補模型，這種能量最可能吸引哪種十神特質的異性？並說明互補邏輯。（例如：吸引偏財男，因他能以「流動、不施加壓力」的方式接住其情緒，讓她不必一直用力輸出。）
  - **吸引的本質**：這份吸引力滿足了主體哪方面的「疲勞」或「內在缺口」？這讓你終於可以不用再 [描述疲勞點]。
...
- **性格特質：** 根據配偶星的十神特質（正官、七殺、正財、偏財等）與五行組合，推斷對方的性格傾向與行為模式。
...
- **關係階段動力推演（基於能量疲勞互補模型）**：
  - **初見·甜蜜期**：雙方互動如何完美體現「能量互補」？會有哪些甜蜜的具體表現？（描述具體互動場景，如何互補）
  - **相處·磨合期**：初期的互補模式，可能固化成怎樣的關係角色？（例如：照顧者 vs. 被照顧者、聆聽者 vs. 表達者）。這種模式可持續的基礎是什麼？
  - **長期·風險點**：隨著現實壓力（經濟、責任、時間）介入，上述互補模式可能在哪個環節崩潰？尤其當一方無法或不再願意提供初始的「互補能量」時，矛盾會如何爆發？最大的隱患在於，當 [觸發條件]，你可能會感到 [核心需求被威脅]，因為TA無法 [持續提供互補]。

- **關係本質綜合評估**：
  - **一句話總結吸引力本質**：例如，「這是一段『表達者』尋找『承接者』的關係。」
  - **長期適配度判斷**：此組合更傾向於「天然互補型」（能形成穩定供需）、「階段課題型」（短期成長，長期需轉化模式），還是「消耗型」（初期吸引強烈，但核心需求本質衝突）？
  - **關鍵維繫條件**：若要發展成長期關係，雙方必須共同修煉或警惕什麼？
