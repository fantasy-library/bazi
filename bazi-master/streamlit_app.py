...
def get_time_period_options():
    """生成时辰下拉菜单选项列表，格式：'子 (23:00-01:00)'，按正确顺序排列"""
    # 确保顺序：子、丑、寅、卯、辰、巳、午、未、申、酉、戌、亥
    zhi_order = ["子", "丑", "寅", "卯", "辰", "巳", "午", "未", "申", "酉", "戌", "亥"]
    options = []
    for zhi in zhi_order:
        if zhi in zhi_time:
            time_range = zhi_time[zhi]
            # 确保使用"丑"而不是"醜"，使用"午"而不是"下午"
            start_hour, end_hour = time_range.split('-')
            start_hour = int(start_hour)
            end_hour = int(end_hour)
            # 格式化时间显示
            start_str = f"{start_hour:02d}:00"
            if end_hour < start_hour:  # 跨日情况（如23-1）
                end_str = f"{end_hour:02d}:00"
            else:
                end_str = f"{end_hour:02d}:00"
            options.append(f"{zhi} ({start_str}-{end_str})")
    return options

def get_zhi_from_option(option):
    """从选项字符串中提取时辰（如从'子 (23:00-01:00)'中提取'子'）"""
    if option and '(' in option:
        return option.split(' (')[0]
    return option if option else "子"

def get_hour_from_zhi(zhi):
    """将时辰转换为对应的小时数（取时间范围的起始小时）"""
    if zhi not in zhi_time:
        return 12  # 默认值
    time_range = zhi_time[zhi]
...
with header_container:
    ...
    # 添加提示注释
    st.markdown(f"""
    <div style="margin: -10px 0 15px 0; padding: 8px 0;">
        <p style="margin: 4px 0; color: #636e72; font-size: 14px;">💡 {T("輸入你的出生時間")}</p>
        <p style="margin: 4px 0; color: #636e72; font-size: 14px;">{T("不清楚出生時辰請先隨意選一個")}</p>
    </div>
    """, unsafe_allow_html=True)
...
## 核心任務
將八字命盤視為「人生股票」，用視覺化數據呈現「基本面(五行)」與「技術面(運勢)」。使用ASCII圖表、Emoji、進度條，讓用戶3秒內看懂。嚴禁長篇大論。

**⚠️ 關鍵原則：** 所有數據、圖表、評分都必須根據實際命盤進行計算，不得直接套用示例數據。示例僅為格式參考，需替換為實際計算結果。
...
2. 月令調整：標註月令對各五行的生扶/剋洩作用
3. 強度判定：過旺/旺/中/弱/極弱
4. 失衡識別：找出最過旺和最過弱的五行

**輸出：** 表格或視覺化呈現五行能量分佈，標註月令狀態與風險點。
...
**輸出：** 六維能力值（0-100），用進度條或等級呈現，附簡要評語。

### 📈【3. 技術面：人生K線圖】

**核心概念：**
- 🟢 綠色=吉運上漲，🔴 紅色=凶運下跌
- 每年K線包含：開盤、收盤、最高、最低（0-100分）
- 必須用 Code Block (```text) 輸出，保持對齊

**(A) 全生命週期大運K線圖**

**⚠️ 重要：** 以下為格式示例，必須根據實際命盤的大運干支、喜忌、起運年齡進行計算繪製。
   ...
   25  |          ╱╲   ╱  ╲ ╱    ╲                    ╲
    0  |[IPO]╱  ╲ ╱    ╲                                ╲___
      __|________________________________________________________
       |初始期|[大運1]|[大運2]|[大運3]|[大運4]|[大運5]|...
       |[干支]|      |      |      |      |      |
年齡:   1   11   21   31   41   51   61   71   81   91
```

**繪製要求：**
- Y軸：運勢分（0, 25, 50, 75, 100）
- X軸：年齡（1, 11, 21, 31...），需根據實際起運年齡調整
- 頂部：實際大運干支，用垂直虛線 | 標記邊界
- 初始期：大運開始前（通常1-8歲），標註實際初始期干支
- K線顏色：🟢 綠色=吉運（大運為喜用），🔴 紅色=凶運（大運為忌神）

**(B) 未來十年流年K線詳解 ({datetime.now().year}-{datetime.now().year + 9})**

**⚠️ 重要：** 以下示例僅為格式參考，必須根據實際命盤、當前年份({datetime.now().year})、大運流年喜忌進行計算，不得直接套用示例數據。

**操作指令簡易對照表：**

| 評分區間 | 趨勢特徵 | 指令 | 核心含義 |
| :--- | :--- | :--- | :--- |
| **80-100** | ↘ 轉跌 / → 見頂 | **賣出** | 運勢達頂峰，獲利了結 |
| **80-100** | ↗ 續漲 / → 高位 | **持有** | 運勢極佳，繼續享受紅利 |
| **60-80** | ↗ 加速上升 | **加倉** | 機會明確，加大投入 |
| **60-80** | ↗ 穩步上升 | **回升** | 漸入佳境，順勢而為 |
| **40-60** | ↗ 低位反彈 | **反彈** | 走出低谷，短期小利 |
| **40-60** | → 震盪 / 不明 | **觀望** | 方向未明，多看少動 |
| **0-40** | → 低位盤整 | **築底** | 運勢低迷，積蓄力量 |
| **0-40** | ↘ 持續下跌 | **止損** | 運勢崩壞，防守為上 |

**⚡ 快速判斷心法：**
1. **高分區 (80+)**：還在漲就「持有」，不漲了就「賣出」。
2. **中高區 (60-80)**：漲勢猛就「加倉」，漲勢穩就「回升」。
3. **中低區 (40-60)**：有起色就「反彈」，沒起色就「觀望」。
4. **低分區 (<40)**：穩住了就「築底」，還在跌就「止損」。
...
年份:  [{datetime.now().year}開始]  [{datetime.now().year + 1}]  [{datetime.now().year + 2}]  ...
操作:  [根據評分選擇]  [根據評分選擇]  [根據評分選擇]  ...
```

**年度K線詳解表（必須根據實際命盤計算）：**

| 年份 | 流年干支 | 年齡 | 大運 | 開盤 | 收盤 | 最高 | 最低 | 顏色 | 判斷 | 提示 |
| :--- | :--- | :--- | :--- | :---: | :---: | :---: | :---: | :---: | :--- | :--- |
| {datetime.now().year} | [計算] | [計算] | [當前大運] | [計算] | [計算] | [計算] | [計算] | 🟢/🔴 | 吉運▲/凶運▼ | [1-2句] |
| {datetime.now().year + 1} | [計算] | [計算] | [大運] | [計算] | [計算] | [計算] | [計算] | 🟢/🔴 | [判斷] | [提示] |

**運勢評分計算：**
- 基礎50分 + 大運喜用+10 / 忌神-10 + 流年喜用+8 / 忌神-8 + 格局+10 / 破壞-10
- 限制在0-100範圍內

### 📉【4. 未來五年運勢趨勢】

**⚠️ 重要：** 必須根據實際命盤計算流年干支、喜忌、與原局大運互動，不得使用占位符。

| 年份 | 流年干支 | 綜合運勢 | 財運 | 事業 | 感情 | 關鍵字 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| {datetime.now().year} | [計算] | [計算] | [計算] | [計算] | [計算] | [計算] |
| {datetime.now().year + 1} | [計算] | [計算] | [計算] | [計算] | [計算] | [計算] |
| {datetime.now().year + 2} | [計算] | [計算] | [計算] | [計算] | [計算] | [計算] |
| {datetime.now().year + 3} | [計算] | [計算] | [計算] | [計算] | [計算] | [計算] |
| {datetime.now().year + 4} | [計算] | [計算] | [計算] | [計算] | [計算] | [計算] |

### 🛠️【5. 系統優化方案】

**🧠 心法重塑：** 針對最弱元素/喜用神，提供思維修正建議（約100字）

**⚡ 行為修正：** 針對忌神/過旺元素，提供日常行為SOP（約100字）

**🎨 環境補強：** 幸運色、幸運方位（增強對應五行氣場）

---

**💡 原則：** 格式是工具，洞察才是目的。可根據命盤特點靈活調整，保持核心邏輯準確。
...
八字元素	企業隱喻	職涯具體對應
日主	創始人兼CEO	核心自我、決策風格、精力值。
印星	研發與文化部 (R&D & HR)	學習力、證照、內在安全感、價值觀、貴人導師。
比劫	合夥與執行部 (Partnership & Ops)	同事、朋友、合作者、競爭對手、體力與執行團隊。
食傷	創新與市場部 (Innovation & Marketing)	創意、表達、技藝、產品開發、個人品牌。
財星	財務與營運部 (Finance & Biz Ops)	資源、財富、現實目標、流程效率、功利驅動力。
官殺	法規與戰略部 (Compliance & Strategy)	規則、壓力、上司、事業野心、社會地位、KPI。
部門互動即職場事件：

食傷生財：你的創意（食傷）能有效變現（財），適合 內容創作、技術變現、自媒體創業。

官印相生：規則（官）帶來權威和資源（印），適合 體制內晉升、考取公職、在大平臺獲得庇廕。

比劫奪財：合夥人/同事（比劫）爭奪你的資源與功勞（財）， 慎選合夥、避免與朋友有財務糾紛。

傷官見官：你的叛逆與才華（傷官）挑戰權威與規則（官），易 頂撞上司、因言獲咎、法律風險高。📤 交付成果：個人職涯戰略報告 (7-Part Strategic Report)
請使用 Markdown 格式，嚴格按以下 七大步驟 交付，確保邏輯連貫，從 診斷 到 執行 一氣呵成。

Part 1: 公司基本面與CEO診斷 (Corporate Foundation & CEO Profile)
公司名稱：{性別}的個人無限公司
...
商業模式 (格局)：{格局，e.g., 食神生財格、殺印相生格}

CEO體質 (身強弱)：{身強/身弱/中和} - 簡述依據：得令？得地？得勢？

核心戰略資源 (用神)：{五行}。解讀：這是公司最需要 引進、投資或依賴 的"部門"或"外部環境"，用以優化運營。

主要營運風險 (忌神)：{五行}。解讀：這是公司最容易 發生內耗、造成虧損或阻礙發展 的"部門"或"外部挑戰"。
...
用一個強有力的商業比喻，定義這家公司的本質。例："這是一家 研發驅動型 (食傷) 的初創企業，但需要引入 頂級風投和法務合規官 (官印) 來規範化並獲取資源，否則容易陷入 創始團隊內訌 (比劫) 而燒光現金流 (財)。"

Part 2: 組織能力六維雷達圖 (Org Capability Radar)
基於八字原局各"部門"的強弱與互動，評估公司在以下核心競爭力上的 先天得分 (0-100)。評分需給出簡短理由。

能力軸	對應部門組合	評分	戰略解讀
領導統御	官殺(權威)+印星(智慧)	XX	制定戰略、帶領團隊、建立秩序的潛力。
地面推進	比劫(團隊)+七殺(攻堅)	XX	執行落地、親力親為、克服具體困難的能力。
創新研發	食傷(創造)+偏印(深鑽)	XX	產生新想法、學習新技術、深度策劃的天賦。
商業變現	財星(資源)+食傷(產品)	XX	將想法或勞動轉化為收入、掌控流程效率的能力。
組織政治	正官(規則)+正財(協調)	XX	在複雜體系內協調人際、平衡利益、穩步上升的能耐。
抗壓韌性	印星(緩衝)+根氣(根基)	XX	承受壓力、應對挫折、在逆境中生存恢復的彈性。
📊 雷達圖核心發現：

王牌競爭力 (最長板)：{能力} ({分數})。這是你的 核心差異化優勢，應在求職和工作中 刻意展示與放大。

戰略短板 (最脆弱環)：{能力} ({分數})。這是你的 系統性風險，必須通過 尋找互補搭檔、選擇合適環境 或 系統性學習 來規避或補強。

Part 3: 企業形態診斷：創業型 vs. 平台型
本部分是 關鍵創新點，直接回答"我適合哪類組織"的問題。
基於原局十神力量對比與格局，判斷此"公司"的先天組織形態：

如果「食傷」、「財星」、「比劫」力量顯著強於「官殺」、「印星」：

形態診斷：【敏捷創業型】企業。

特質：產品/技術導向，反應快速，厭惡僵化流程，追求短期市場驗證和增長。內部管理可能鬆散，但創新活力強。

適合環境：初創公司、自由職業、自媒體、銷售、藝術創作、技術研發等 自由度與變動性高 的領域。

風險提示：在等級森嚴、規則繁多的 大型傳統組織或政府部門 中，會感到 極度壓抑，容易觸發"傷官見官"衝突。

如果「官殺」、「印星」力量顯著強於「食傷」、「財星」：

形態診斷：【穩健平台型】企業。

特質：重視規則、結構、長期穩定與安全保障。擅長在既有體系內利用規則和資源（印）獲得晉升（官）。創新和冒險精神較弱，但抗風險能力強。

適合環境：政府機關、公立學校、大型國企/外企、金融機構、軍隊 等 階級明確、制度健全 的平臺。

風險提示：在需要顛覆性創新、野蠻生長的 純市場化初創環境 中，可能 決策緩慢，難以適應。

如果力量相對均衡或有特殊格局：

形態診斷：【混合適應型/專業服務型】企業。

特質：能在規則與創新之間取得平衡。可能適合 專業服務機構（如諮詢、法律、會計）、大型企業的創新部門、有技術門檻的穩定行業。

策略：需要進一步看 大運引動 哪個部門，來決定階段性側重。

Part 4: 戰略賽道選擇 (Strategic Roadmapping)
結合 用神（所需資源）、企業形態、核心優勢，規劃發展路徑。

🎯 主攻賽道 (高度契合 - 建議All In)：

{賽道A}：原因（為何你的"公司形態"和"王牌競爭力"匹配此賽道）。具體職能舉例。成長路徑預覽（從入行到成熟的關鍵節點）。

{賽道B}：原因（作為備選或相關賽道）。具體職能舉例。與賽道A的協同效應。

📈 機會賽道 (可以嘗試 - 需創造條件)：

{賽道C}：原因（部分能力匹配，但形態不完全契合）。成功前提（需補強何種能力、尋找何種合作夥伴、選擇該行業中的哪類特定公司）。

🚫 禁止進入賽道 (嚴重不匹配 - 規避為上)：

{賽道D}：原因（與你的"企業形態"直接衝突，或會激發"致命坑點"）。預測後果（在此賽道中可能遭遇的典型失敗場景）。

Part 5: 五年戰略週期與年度OKR (5-Year Strategic Timeline)
將大運流年轉化為 市場週期預判，制定 年度關鍵目標與行動。

財年 (年齡)	市場週期 (大運+流年)	綜合風險評級	年度戰略主題	年度關鍵行動 (OKR)
{YYYY}	{大運干支} + {流年干支}	🟢 擴張期 / 🟡 調整期 / 🔴 收縮期	{e.g., 築基、躍升、防守}	1. O1：{目標} -> KR：{可量化結果}
2. O2：{目標} -> KR：{可量化結果}
{YYYY+1}	{大運干支} + {流年干支}	🟢 / 🟡 / 🔴	{e.g., 合作、深耕、轉型}	1. O1：{目標} -> KR：{...}
{YYYY+2}	{大運干支} + {流年干支}	🟢 / 🟡 / 🔴	{e.g., 壓力測試、現金流管理}	1. O1：{風險防範目標} -> KR：{...}
🎯 近期（1-3年）戰略進攻窗口：

關鍵時點：{具體年份與季度，如2025年Q3-Q4}。

窗口性質：{是跳槽黃金期、進修視窗、還是創業啟動點？}。

戰前準備清單：在視窗開啟前，必須完成 {技能儲備}、{人脈鋪墊}、{資金準備}。

Part 6: 戰術執行手冊 (Tactical Playbook)
🟢 本月速勝行動 (30天內，建立動能)：

{行動1}：針對"王牌競爭力"做一次 小的公開展示（如寫一篇專業短文、優化作品集首頁）。

{行動2}：開始一項 微習慣，以補充"戰略資源"（用神）。如喜"印"則每日閱讀15分鐘專業報告。

{行動3}：切斷一個助長"主要營運風險"（忌神）的習慣。如忌"比劫"則精簡一個無效社交群。

🎯 本季度戰略專案 (90天，取得階段成果)：

專案名稱：{例如：完成"XX技能"認證，併發起3次目標行業的資訊訪談}。

成功標準 (KPI)：{量化指標，如：拿到證書，新增2位有效行業人脈}。

📈 半年度戰略覆盤 (180天，校準航向)：

覆盤會議議程：對比 Part 5 的年度OKR，檢查進度。重點反思：是否因"戰略短板"而受阻？是否抓住了"進攻視窗"的訊號？

Part 7: 風險管理與應急預案 (Risk Management)
⚠️ 高發性組織風險預警：

風險類別	預期觸發週期	早期預警訊號	應急預案與對沖策略
{風險1：如團隊叛離/合夥糾紛}	{相關大運/流年}	核心夥伴頻繁抱怨利益分配，溝通效率下降。	1. 法律準備：複核協議。
2. B計畫啟動：接觸潛在替代合作方。
3. 業務聚焦：收縮戰線，確保核心現金流。
{風險2：如合規危機/上司打壓}	{相關大運/流年}	收到模糊的警告，感到被規則刻意針對，專案受阻。	1. 尋求庇護：主動向更高層或HR（印）透明溝通。
2. 嚴格合規：所有操作留下紙面痕跡。
3. 低調蟄伏：暫停激進創新，專注完成本職KPI。
🧭 顧問最終戰略心法：

用一句鏗鏘有力、易於記憶的話，概括整個戰略報告的精髓，作為客戶日常決策的"北極星"。例如："對你而言，'在巨人的肩膀上做減法' 是上策——加入一個穩健平臺（官印），然後在其中找到一個能用你創意（食傷）解決具體問題的利基市場，遠離複雜的股權合夥（比劫）。請準備好，現在請接收用戶的命盤資訊："
""")
        # 复制到剪贴板按钮 - 職涯規劃版
...
with st.container():
    ...
    with date_container:
        ...
        with date_cols[3]:
            st.markdown(f'<div style="text-align: center; color: #666; margin-bottom: 5px; font-weight: 500;">{T("時辰")}</div>', unsafe_allow_html=True)
            # 时辰下拉菜单选项
            time_period_options = get_time_period_options()
            # 默认选择"午"（对应12点左右）
            default_index = time_period_options.index("午 (11:00-13:00)") if "午 (11:00-13:00)" in time_period_options else 6
            ...
            # 从选择的时辰中提取时辰字符并转换为小时数
            selected_zhi = get_zhi_from_option(selected_time_period)
            hour = get_hour_from_zhi(selected_zhi)
            # 系统自动处理分钟，设置为0
            minute = 0
