...
# 全局清理：移除內容中形如 1-158 的數字範圍標註
try:
    import re as _re
    _range_re = _re.compile(r"\b\d{1,3}-\d{1,3}\b")
    if isinstance(globals().get('summarys'), dict):
        for _k, _v in list(summarys.items()):
            if isinstance(_v, str):
                summarys[_k] = _range_re.sub('', _v)
except Exception:
    pass
